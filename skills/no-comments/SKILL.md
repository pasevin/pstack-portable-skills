...
1. Spawn a fresh subagent, isolated from this conversation's context, and give it the full persona and rules in `references/comment-sicko.md` as its instructions, plus the scope. Do not restate its rules to it beyond that file. If your environment has no subagent-delegation mechanism, adopt the persona yourself for a dedicated, isolated review pass instead.
